...
# If attempting to pull in files from ai-winter repo folder copied into this project's 
# directory (e.g. path: C:\Users\...\IntroToRobotics_Project\controllers\youbot_thetaStar)
# you may need:
...
Otherwise, use internal imports for local supporting files
"""

# from local supporting files:
from planner_core import OccupancyGrid, theta_star
from factory_map import build_factory_map, visualize_factory_map
...
def get_robot_pose():
    ...
    Webots normally uses X-Z as the horizontal ground plane.
    However, the world file for this project (factory.wbt)sets
    X-Y as the horizontal ground plane. So we map:
    ...
    # the rotation matrix entries related to X-Y heading.
    # Rotation matrix returned row-major:
    # [r00 r01 r02
    #  r10 r11 r12
    #  r20 r21 r22]
    #
    # For motion in the Webots X-Y plane, yaw is rotation about Z.
...
# -----------------------------
def log(msg):
    print(msg)  # keep original console output
...
def draw_log_overlay():
    ...
    display.setColor(0x00FF00)  # green text
...
log_lines = []            # log console text to display
MAX_LOG_LINES = 12
...
def display_helper_message():
    ...
    log("\n\nControl commands:")
    log(" Arrows:         Move the robot")
    log(" Page Up/Down:   Rotate the robot")
    log(" +/-:            (Un)grip")
    log(" Shift + arrows: Handle the arm")
    log(" Space:          Reset")
    log(" A:              Toggle autonomous mode (placeholder)")
...
def base_move(vx, vy, omega):
    ...
    log(f"Speeds: vx={vx:.2f} [m/s], vy={vy:.2f} [m/s], omega={omega:.2f} [rad/s]")
...
def arm_set_height(height):
    ...
    if height == ARM_FRONT_FLOOR:
        ...
        log("arm_set_height() called with wrong argument")
        return
...
def arm_set_orientation(orientation):
    ...
    if orientation == ARM_BACK_LEFT:
        ...
        log("arm_set_orientation() called with wrong argument")
        return
...
def autonomous_step():
    ...
    if not path_planned:
        ...
        if not path_cells:
            ...
            log("Theta* failed: no path found.")
            base_reset()
            path_planned = True
            return
    ...
    # Convert world-frame waypoint error into robot/body-frame command
    ...
    vx = clamp(k_xy * dx, -MAX_SPEED, MAX_SPEED)
    vy = clamp(k_xy * dy, -MAX_SPEED, MAX_SPEED)
    omega = 0.0
    ...
    # Direction from robot to target in world frame
    ...
    print(
        ...
        f"d=({dx:.2f}, {dy:.2f}, dist={distance:.2f}) "
...
#    base_move(0.0, 0.1, 0.0) should strafe robot-local left/right
#    but it's not working, so the code above avoids the failing 
#    strafe mode entirely by setting vy = 0.0 
#    (ie: no lateral/holonomic motion, which Mecanum wheels usually do)
...
"""
#
build_factory_map(grid_map)

# Inflate obstacles so the youBot does not clip corners:
grid_map.inflate_obstacles(0.50)
...
# Print 2D factory map (ie: floor plan for factory.wbt):
visualize_factory_map(grid_map, goal=goal_world)
