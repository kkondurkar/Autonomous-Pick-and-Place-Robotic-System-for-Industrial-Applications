...
BOX_FOUND_BACKUP_SECONDS = 0.5

CABINET_CENTER_TOLERANCE_PX = 5
CABINET_RECENTER_TOLERANCE_PX = 25

# This uses the same camera-recognition axis your working box code uses: pos[0].
# If cabinet approach behaves backwards or never arrives, try using pos[2] instead
# in process_box_pickup_state_machine().
CABINET_APPROACH_DISTANCE = 0.45

# Tune these three values for your cabinet shelf/opening.
...
# -----------------------------
# Box-to-cabinet state machine modes
# -----------------------------
MODE_SEARCH = 0
MODE_CENTER = 1
MODE_APPROACH = 2
MODE_GRAB = 3
MODE_SEARCH_CABINET = 4
MODE_CENTER_CABINET = 5
MODE_APPROACH_CABINET = 6
MODE_PLACE_IN_CABINET = 7
MODE_DONE = 8
...
def display_helper_message():
    ...
    print(" B:              Toggle automatic box-to-cabinet routine")
...
def get_object_model(obj):
    ...
    Some recognized objects can have invalid/non-UTF-8 model bytes, especially
    if recognitionColors were added to generated child solids inside a PROTO.
    This prevents one bad object from crashing the whole controller.
    """
    if hasattr(obj, "getModel"):
        try:
            model = obj.getModel()
            ...
            # Webots stores the raw model bytes in _model in some versions.
            raw = getattr(obj, "_model", b"")
            if isinstance(raw, bytes):
                return raw.decode("utf-8", errors="ignore")
            return ""
        except Exception as exc:
            print(f"Could not read recognition model: {exc}")
            return ""
...
def grab_box_sequence():
    ...
    # The C code had this movement commented out.
    ...
    # gripper_release()
    # arm_set_height(ARM_BACK_PLATE_GRAB)
    # passive_wait(3.0)
    # gripper_grip()
    # passive_wait(1.0)
    # arm_set_height(ARM_RESET)
...
def back_up_after_finding_box():
    """Back up slightly after first detecting the box to avoid collisions."""
    print("Box found. Backing up slightly before approach.")
    base_reset()
    passive_wait(0.2)
...
def place_box_in_cabinet_sequence():
    """Run after the box is already held in the gripper.

    This assumes the robot has already centered on the cabinet and stopped at
    CABINET_APPROACH_DISTANCE. It then raises/holds the box, nudges forward,
    releases it, and backs away.
    """
    base_reset()

    # Use the same front-facing orientation used by the box pickup routine.
    arm_set_orientation(ARM_FRONT)
    arm_set_height(CABINET_PLACE_HEIGHT)
    passive_wait(2.0)

    # Insert slightly into the cabinet. Tune CABINET_INSERT_SECONDS.
    base_forwards()
    ...
    # Back out of the cabinet opening.
    base_backwards()
    passive_wait(CABINET_BACK_OUT_SECONDS)
    base_reset()

    # Return to a safe carrying/resting pose.
    arm_reset()
    passive_wait(1.0)
...
def approach_target(error_x, rel_x, rel_z, recenter_tolerance_px, stop_distance, label):
    if abs(error_x) > recenter_tolerance_px:
        print(f"Recentering on {label}")
        return "recenter"

    # This intentionally uses rel_x because your working box approach uses rel_x.
    # If the cabinet never reaches the stop distance, test rel_z here instead.
    if rel_x > stop_distance:
        print(f"Approaching {label}, rel_x={rel_x:.3f}, rel_z={rel_z:.3f}, stop={stop_distance:.3f}")
...
def process_box_pickup_state_machine(objects):
    """Full routine: find box, grab box, find cabinet, place box in cabinet."""
    global mode
    ...
    # -----------------------------
    # 1) Existing working box routine
    # -----------------------------
    if mode in (MODE_SEARCH, MODE_CENTER, MODE_APPROACH, MODE_GRAB):
        ...
        if mode == MODE_SEARCH:
            ...
            back_up_after_finding_box()
            # This is the important change: do not finish after grabbing.
            # The box is now held by the robot, so start searching for the cabinet.
            mode = MODE_SEARCH_CABINET

        return

    # -----------------------------
    # 2) New cabinet routine
    # -----------------------------
    if mode in (MODE_SEARCH_CABINET, MODE_CENTER_CABINET, MODE_APPROACH_CABINET, MODE_PLACE_IN_CABINET):
        cabinet = find_cabinet(objects)

        if cabinet is None:
            mode = MODE_SEARCH_CABINET
            print("No cabinet detected")
            base_turn_left_increment()
            return
        ...
        print(
            f"Cabinet img_x={x} error_x={error_x} "
            f"rel=({pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f})"
        )

        if mode == MODE_SEARCH_CABINET:
            mode = MODE_CENTER_CABINET

        elif mode == MODE_CENTER_CABINET:
            if center_target_on_image(error_x, CABINET_CENTER_TOLERANCE_PX):
                mode = MODE_APPROACH_CABINET

        elif mode == MODE_APPROACH_CABINET:
            result = approach_target(
                error_x,
                rel_x,
                rel_z,
            ...
            if result == "recenter":
                ...
                mode = MODE_PLACE_IN_CABINET

        elif mode == MODE_PLACE_IN_CABINET:
...
def handle_keyboard(key, previous_key):
    ...
    if key == Keyboard.UP:
        ...
        print(f"Automatic box-to-cabinet routine: {'ON' if box_pickup_enabled else 'OFF'}")
